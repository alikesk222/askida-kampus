...
<div class="mb-6">
    <a href="<?= url('bagis') ?>" class="text-[#00A3B4] text-sm hover:underline">← <?= t('donor.donate_title') ?></a>
    <h1 class="text-2xl font-bold text-gray-800 mt-2"><?= e($venue['name']) ?> — <?= t('donor.donate_title') ?></h1>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <form method="POST" action="<?= url('bagis/' . $venue['id']) ?>" id="donation-form">
            ...
            <div class="bg-white rounded-xl shadow-sm overflow-hidden mb-4">
                <div class="px-6 py-4 border-b border-gray-100">
                    <h2 class="font-semibold text-gray-700"><?= t('donor.select_products') ?></h2>
                    <p class="text-xs text-gray-400 mt-0.5"><?= t('donor.select_sub') ?></p>
                </div>
                <div class="divide-y divide-gray-100">
                    ...
                    <div class="px-6 py-4 flex items-center justify-between gap-4">
                        ...
                        <div class="text-right">
                            ...
                            <p class="text-xs text-gray-400"><?= t('common.per_unit') ?></p>
                        </div>
                        <div class="w-24">
                            <label class="block text-xs text-gray-500 mb-1 text-center"><?= t('donor.qty_label') ?></label>
            ...
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700 mb-1"><?= t('donor.note_lbl') ?></label>
                <textarea name="notes" rows="2"
                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#00A3B4]"
                    placeholder="<?= t('donor.note_ph') ?>"><?= old('notes') ?></textarea>
            </div>
    ...
    <div class="space-y-4">
        <div class="bg-white rounded-xl shadow-sm p-5">
            <h3 class="font-semibold text-gray-700 mb-3"><?= t('donor.total_amount') ?></h3>
            <p id="total-amount" class="text-3xl font-bold text-[#00A3B4]">₺0,00</p>
            <p class="text-xs text-gray-400 mt-1"><?= t('donor.total_sub') ?></p>
        </div>
        ...
        <div class="bg-[#E0F7FA] border border-[#00A3B4]/30 rounded-xl p-5">
            ...
            <p class="text-xs text-[#007A8A] mt-3"><?= t('donor.iban_after') ?></p>
        </div>
    </div>
</div>
<?php endif; ?>
